...
namespace SteamworksPlus.Runtime.Providers.Facepunch.Extentions
{
	...
	/// </summary>
	public static class SPLobbyExtentions
	{
		/// <summary>
		...
		/// <param name="result">The result returned a default value</param>
		/// <returns>The success of the operation</returns>
		public static bool GetUlongValueFromLobbyMemberData(this Lobby lobby, Friend member, string key, out ulong result)
        {
            return ulong.TryParse(lobby.GetMemberData(member ,key), out result);
        }
        ...
        /// <param name="result">The result returned or a null value</param>
        /// <returns>The success of the operation</returns>
        public static bool GetTValueFromLobbyMemberData<T>(this Lobby lobby, Friend member, string key, out T? result)
            where T : class
        {
            try
            {
                result = JsonUtility.FromJson(lobby.GetMemberData(member, key), typeof(T)) as T;
                return true;
            }
            catch (Exception)
            {
